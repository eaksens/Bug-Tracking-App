...
(function () {
  ...
  var insertUsers = function(users) {
    ...
    users.forEach(function(user){
      ...
      var newText = document.createTextNode(user.lastName + ", " + user.firstName + ": " + user.email + "," + user.thumbnail);
  ...
  var xmlToUsers = function (userResponseXML) {
    ...
    for (var i = 0; i < userListXML.length; i++) {
      ...
      var user = {
        ...
        thumbnail: userXML.getElementsByTagName("email")[0].textContent
      };
      users.push(user);
  ...
  var loadUsers = function () {
    ...
    ajax.open("GET", 'https://bug1-tracker.herokuapp.com/users.xml');
